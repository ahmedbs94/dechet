...
# ═════════════════════════════════════════════════════════════════════════════
# NŒUD 1 : /utilisateurs/{qr_code}
# Cle = QR code unique (ex: TRIDECHET-aaa433a8-7683-4574-8fa7-64b600916f0c)
# ═════════════════════════════════════════════════════════════════════════════

def sync_utilisateurs(db) -> None:
    """
    Synchronise tous les utilisateurs dans /utilisateurs/{qr_code}/.
    Cle du noeud = QR code unique (pas l'ID numerique).
    Structure :
      - id        : int  (identifiant PostgreSQL)
    ...
    for u in users:
        ...
        # Le champ "score" n'existe QUE pour les citoyens
        if role == "user":
            data["score"] = round(float(getattr(u, 'global_score', 0) or 0), 2)

        # Cle = qr_code (ex: TRIDECHET-aaa433a8-...)
        ok = _firebase_set(f"utilisateurs/{qr}", data)
        if ok:
            STATS[node]["ok"] += 1
        else:
            STATS[node]["err"] += 1

    print(f"[UTILISATEURS] ✓ {STATS[node]['ok']} synchro(s)  ✗ {STATS[node]['err']} erreur(s)  ⏩ {STATS[node]['skip']} ignoré(s)")
